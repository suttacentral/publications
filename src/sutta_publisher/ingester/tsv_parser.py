...
class TsvParser(BaseParser):
    def __init__(self, tsv: str | PathLike[str] | ReadCsvBuffer[bytes] | ReadCsvBuffer[str]) -> None:
        ...
        self.spreadsheet = self.spreadsheet[["english", "html", "reference", "segment_id"]]
        self.spreadsheet.dropna(axis=0, how="all", inplace=True)
    ...
    def parse_input(self) -> str:
        ...
        # Prepare output (HTML element with line of sutta) from each row of the dataframe
        for _, row in self.spreadsheet.iterrows():
            html: str = row["html"] if row["html"] is not NaN else "{}"  # sometimes there are records with missing HTML
            ...
            segment_id: str = _segment_id_to_html(row["segment_id"]) if row["segment_id"] is not NaN else ""
            # e.g.      <p><a class='sc-main' id='dn1:1.10.16'>DN 1:1.10.16</a>‘He refrains from running errands...<a id='dn1:1.10.17'></a></p>
            output.append(html.format(f"{reference} {row['english']}{segment_id}"))
