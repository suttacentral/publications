...
def test_generate_request_headers(bot_api_key: str):
    header = github_handler.__get_request_headers(bot_api_key)

    assert header["Authorization"] == f"token {bot_api_key}"


def test_generate_request_body(monkeypatch, file_like_edition: BytesIO, edition_path_in_repo: str, repo_url: str):
...
def test_raise_attribute_error(monkeypatch, edition_path_in_repo: str, repo_url: str):
    monkeypatch.setattr(github_handler, "__get_file_sha", lambda *args: "someshanumber")
...
def test_raise_type_error(monkeypatch, edition_path_in_repo: str, repo_url: str):
    monkeypatch.setattr(github_handler, "__get_file_sha", lambda *args: "someshanumber")
